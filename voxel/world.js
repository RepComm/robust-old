...
import { ParticleSystem } from "../particle/particle.js";
...
export class World extends Object2D {
  ...
  constructor() {
    ...
    this.addWeatherMode("rainy", {
      ...
      particleSettings: {
        ...
        spawnInterval: 250,
        spawnGenerator: ps => {
          ps.spawnParticle(Math.random() * 32, 0, -0.1, 0.4 + Math.random() / 3);
        }
      }
    });
    this.addWeatherMode("snowy", {
      ...
      particleSettings: {
        ...
        spawnInterval: 250,
        spawnGenerator: ps => {
          ps.spawnParticle(Math.random() * 32, 0, 0, 0.1 + Math.random() / 4);
        }
      }
    });
  ...
  setChunkActivated(chunk, activate = true) {
    if (activate) {
  ...
  cacheChunk(x, y) {
    this.cachedChunk = this.getChunk(Chunk.blockXToChunkIndexX(x), Chunk.blockYToChunkIndexY(y));
  ...
  getBlock(x, y, out) {
    ...
    this.cachedChunk.getBlock(Chunk.blockWorldXToBlockChunkX(x), Chunk.blockWorldYToBlockChunkY(y), out);
    out.worldX = x;
    out.worldY = y;
    return true;
  }
  ...
  setBlock(x, y, block, doEvent = true, doParticle = true, recalcCollision = true) {
    this.setBlockCurrent.copy(block);
    if (!this.getBlock(x, y, this.setBlockPrevious)) return false; //If blocks are the same, don't do all that processing..
    ...
    if (doParticle) {
      //Play particle
      this.psBlockSet.spawnParticle(x + 0.5, y + 0.5, 0, 0, this.setBlockPrevious.type);
    }
