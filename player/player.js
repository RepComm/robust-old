...
import { PhysicsObject2D } from "../physics/pobj.js";
export class Player extends PhysicsObject2D {
  ...
  render(ctx) {
    ...
    ctx.stroke();
    ctx.strokeRect(0, 0, this.aabb.size.x, this.aabb.size.y);
    ctx.fillStyle = "white";
    ctx.font = `${0.5}px courier`;
    ctx.fillText(`${Math.floor(this.transform.position.x)}, ${Math.floor(this.transform.position.y)}`, this.aabb.size.x, this.aabb.size.y);
